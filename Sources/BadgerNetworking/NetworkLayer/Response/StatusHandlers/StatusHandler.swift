...
class StatusHandler<U: JSONParsable>: ResponseStatusHandler {
    typealias T = U
    ...
    var handleResponse: ((inout NetworkResponse<T>) -> Void)?
    
    init(status: [Range<Int>], handler: @escaping (inout NetworkResponse<T>) -> Void) {
        self.status = status
